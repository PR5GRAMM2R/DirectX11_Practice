...
#pragma omp parallel for
	for (int j = 0; j < this->height; j++)
	{
		for (int i = 0; i < this->width; i++)
		{
			...
			pixelsBuffer[i + this->width * j].v[0] = (float)r / 5;
			pixelsBuffer[i + this->width * j].v[1] = (float)g / 5;
			pixelsBuffer[i + this->width * j].v[2] = (float)b / 5;
		}
	}
...
#pragma omp parallel for
	for (int j = 0; j < this->height; j++)
	{
		for (int i = 0; i < this->width; i++)
		{
			...
			for (int k = 0; k < 5; k++) {
				Vec4 temp = GetPixel(i + k - 2, j);

				r += temp.v[0] * weights[k];
				g += temp.v[1] * weights[k];
				b += temp.v[2] * weights[k];
			}

			pixelsBuffer[i + this->width * j].v[0] = r;
			pixelsBuffer[i + this->width * j].v[1] = g;
			pixelsBuffer[i + this->width * j].v[2] = b;
		}
	}
	...
	*/
	for (int j = 0; j < height; j ++)
		for (int i = 0; i < width; i++)
		{
			...
			if (0.2126 * temp.v[0] + 0.7152 * temp.v[1] + 0.0722 * temp.v[2] < th) {
				temp.v[0] = 0.0f;
				temp.v[1] = 0.0f;
				temp.v[2] = 0.0f;
			}
