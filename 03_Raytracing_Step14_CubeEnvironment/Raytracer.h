...
namespace hlab
{
	...
	public:
		...
		shared_ptr<Eye> eye = std::make_shared<Eye>(vec3(0.0f, 0.3f, 0.0f));
			...
			: width(width), height(height)
		{
			...
			auto sphere1 = make_shared<Sphere>(vec3(-2.0f, 1.0f, 2.0f), 1.0f); // + 기준, (오른쪽, 위쪽, 앞쪽)

			sphere1->amb = vec3(1.0f);
			...
			auto sphere2 = make_shared<Sphere>(vec3(2.0f, 2.0f, 2.0f), 1.0f);

			sphere2->amb = vec3(1.0f);
			sphere2->dif = vec3(1.0f, 1.0f, 0.0f);
			...
			auto sphere3 = make_shared<Sphere>(vec3(0.0f, 0.0f, -5.0f), 2.0f);

			sphere3->amb = vec3(1.0f);
			sphere3->dif = vec3(1.0f, 1.0f, 0.0f);
			...
			auto ground = make_shared<Square>(vec3(-3.0f, -2.0f, 3.0f), vec3(3.0f, -2.0f, 3.0f), vec3(3.0f, -2.0f, -3.0f), vec3(-3.0f, -2.0f, -3.0f),
			...
			auto frontTexture = std::make_shared<Texture>("./SaintPetersBasilica/posz.jpg"); 
			auto front = make_shared<Square>(vec3(-10.0f, 15.0f, 10.0f), vec3(10.0f, 15.0f, 10.0f), vec3(10.0f, -5.0f, 10.0f), vec3(-10.0f, -5.0f, 10.0),
				vec2(0.0f, 0.0f), vec2(1.0f, 0.0f), vec2(1.0f, 1.0f), vec2(0.0f, 1.0f));
			...
			auto backTexture = std::make_shared<Texture>("./SaintPetersBasilica/negz.jpg"); 
			...
			auto leftTexture = std::make_shared<Texture>("./SaintPetersBasilica/negx.jpg");
			...
			auto rightTexture = std::make_shared<Texture>("./SaintPetersBasilica/posx.jpg"); 
			...
			auto topTexture = std::make_shared<Texture>("./SaintPetersBasilica/posy.jpg");
			auto top = make_shared<Square>(vec3(-10.0f, 15.0f, -10.0f), vec3(10.0f, 15.0f, -10.0f), vec3(10.0f, 15.0f, 10.0f), vec3(-10.0f, 15.0f, 10.0f),
				vec2(0.0f, 0.0f), vec2(1.0f, 0.0f), vec2(1.0f, 1.0f), vec2(0.0f, 1.0f));
			...
			auto bottomTexture = std::make_shared<Texture>("./SaintPetersBasilica/negy.jpg");
			auto bottom = make_shared<Square>(vec3(-10.0f, -5.0f, 10.0f), vec3(10.0f, -5.0f, 10.0f), vec3(10.0f, -5.0f, -10.0f), vec3(-10.0f, -5.0f, -10.0f),
				vec2(0.0f, 0.0f), vec2(1.0f, 0.0f), vec2(1.0f, 1.0f), vec2(0.0f, 1.0f));
			...
			light = std::make_shared<Light>(vec3(0.0f, 0.5f, -0.5f)); // 화면 뒷쪽
		...
		vec3 traceRay(Ray& ray, const int recurseLevel)
		{
			...
			if (hit.d >= 0.0f)
			{
				...
				if (hit.obj->transparency)
				{
					...
					const float sinTheta2 = (float)sinTheta1 / eta;
					const float cosTheta2 = glm::sqrt(1 - glm::pow(sinTheta2, 2));

					const vec3 m = glm::normalize(ray.dir + normal * dot(-ray.dir, normal));
					const vec3 a = cosTheta2 * -normal;
					const vec3 b = sinTheta2 * m;
					const vec3 refractedDirection = glm::normalize(a + b); // transmission'

					Ray refractedRay{ hit.point + 1e-4f * refractedDirection, refractedDirection };
		...
		void Render(std::vector<glm::vec4>& pixels)
		{
			...
			/*shared_ptr<Eye> eye = std::make_shared<Eye>(vec3(0.0f, 0.0f, -1.5f));*/

			static int i = 0;

			if (i == 0) {
				if (this->eye->pos.z > 0)
					this->eye->pos.z = pow(this->eye->pos.z, 2);
				else
					this->eye->pos.z = -pow(this->eye->pos.z, 2);
			}
			i++;
...
#pragma omp parallel for
			for (int j = 0; j < height; j++)
				for (int i = 0; i < width; i++)
				{
					...
					Ray pixelRay{ pixelPosWorld, glm::normalize(pixelPosWorld - this->eye->pos) };
