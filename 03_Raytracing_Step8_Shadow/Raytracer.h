	...
	public:
		...
		vec3 traceRay(Ray &ray)
		{
			...
			if (hit.d >= 0.0f)
			{
				...
				// Shadow
				const vec3 shadowDir = glm::normalize(light.pos - hit.point);
				
				Ray shadowRay = { hit.point + shadowDir * 0.0001f, shadowDir };

				const auto shadowHit = FindClosestCollision(shadowRay);

				if (shadowHit.d >= 0.0f && shadowHit.d <= glm::length(light.pos - shadowRay.start)) {
					return vec3(0.0f);
				}				
...
#pragma omp parallel for
			for (int j = 0; j < height; j++)
				for (int i = 0; i < width; i++)
				{
					...
					// Ray pixelRay{ pixelPosWorld, glm::vec3(0.0f, 0.0f, 1.0f) };
