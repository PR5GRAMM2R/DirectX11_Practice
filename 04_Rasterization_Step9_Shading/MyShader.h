...
vec3 BlinnPhong(vec3 lightStrength, vec3 lightVec, vec3 normal, vec3 toEye,
    ...
    /* (lightVec + toEye) / glm::length(lightVec + toEye);*/

    // Halyway vector를 이용해서 specular albedo 계산
    vec3 specular = std::pow(glm::max(glm::dot(normal, halfway), 0.0f), mat.shininess) * mat.specular;

    // lightStrength 가 diffuse

    // ambient, diffuse, specular 합쳐서 계산
    /*return glm::clamp(mat.ambient + lightStrength + specular, 0.0f, 1.0f);*/
...
vec3 ComputeDirectionalLight(Light L, Material mat, vec3 normal, vec3 toEye) {
    ...
    float ndotl = glm::max(glm::dot(normal, lightVec), 0.0f);
