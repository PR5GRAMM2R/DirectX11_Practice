...
namespace hlab
{
	...
	public:
		...
		vec3 InterpolateBilinear(
			...
			const vec3 &c11)
		{
			glm::vec3 result = ((1 - dx) * (1 - dy)) * c00 + (dx * (1 - dy)) * c10 + ((1 - dx) * dy) * c01 + (dx * dy) * c11;
		...
		vec3 SamplePoint(const vec2 &uv) // Nearest sampling이라고 부르기도 함
		{
			...
			vec2 xy = glm::vec2(-0.5f + uv.x * width, -0.5f + uv.y * height);
		...
		vec3 SampleLinear(const vec2 &uv)
		{
			...
			// std::cout << floor(-0.3f) << " " << int(-0.3f) << std::endl; // -1 0
			 ...
			 const vec2 xy = glm::vec2(-0.5f + uv.x * width, -0.5f + uv.y * height);

			 const int i = glm::floor(xy.x);
			 const int j = glm::floor(xy.y);
