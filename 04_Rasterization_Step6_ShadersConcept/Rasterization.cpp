...
void Rasterization::Update() {
    ...
    for (auto &mesh : this->meshes) {
        ...
        /*for (int i = 0; i < mesh->vertexBuffer.size(); i++) {
            mesh->vertexBuffer[i] =
                vec3(mesh->vertices[i].x * std::cos(mesh->rotationZ) -
                         mesh->vertices[i].y * std::sin(mesh->rotationZ),
                     mesh->vertices[i].x * std::sin(mesh->rotationZ) +
                         mesh->vertices[i].y * std::cos(mesh->rotationZ),
                     mesh->vertices[i].z);
        }*/
    }
}
