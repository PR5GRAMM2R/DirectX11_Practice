...
void Rasterization::Render(vector<vec4> &pixels) {
    ...
    for (size_t i = 0; i < earth.vertexBuffer.size(); i++) {
        auto temp = earth.vertexBuffer[i] + vec3(distSunToEarth, 0, 0);

        this->vertexBuffer[i] = RotateAboutZ(temp, earthAngle);//TODO: 추가
    }
    ...
    for (size_t i = 0; i < moon.vertices.size(); i++) {
        //auto temp1 = moon.vertexBuffer[i] + vec3(distEarthToMoon, 0, 0);
        //temp1 = RotateAboutZ(temp1, moonAngle);

        //auto temp2 = earth.vertexBuffer[0] + vec3(distSunToEarth, 0, 0);
        //temp2 = RotateAboutZ(temp2, earthAngle);

        //this->vertexBuffer[i] = temp1 + temp2;//TODO: 추가
