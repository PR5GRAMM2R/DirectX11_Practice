...
public:
	...
	glm::vec2 TransformScreenToWorld(glm::vec2 positionScreen)
	{
		...
		float x_float = (float)(2 * positionScreen.x - width) / height;
		float y_float = -(float)(2 * positionScreen.y - height) / height;

		/*float x_float = (float)(positionScreen.x - (width - 1) / 2) / width * aspect;
		float y_float = -(float)(positionScreen.y - (height - 1) / 2) / height;*/

		return glm::vec2(x_float , y_float);
	}
