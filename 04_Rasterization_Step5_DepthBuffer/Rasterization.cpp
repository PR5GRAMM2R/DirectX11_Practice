    ...
    for (size_t j = yMin; j <= yMax; j++) {
        for (size_t i = xMin; i <= xMax; i++) {
            ...
            if (alpha0 >= 0.0f && alpha1 >= 0.0f && alpha2 >= 0.0f) {
                ...
                const float depth = (alpha0 * this->vertexBuffer[i0].z + alpha1 * this->vertexBuffer[i1].z + alpha2 * this->vertexBuffer[i2].z) / area;
...
void Rasterization::Render(vector<vec4> &pixels) {
    ...
    std::fill(&depthBuffer[0], &depthBuffer[depthBuffer.size() - 1], 1000.0f);
