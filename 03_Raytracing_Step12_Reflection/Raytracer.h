			...
			if (hit.d >= 0.0f)
			{
				...
				if (FindClosestCollision(shadowRay).d < 0.0f)
				{
					...
					if (hit.obj->reflection)
					{
						...
						const vec3 reflectedDirection = glm::normalize(ray.dir - 2.0f * (glm::dot(ray.dir, hit.normal) * hit.normal));

						Ray reflectedRay{ hit.point + 0.0001f * reflectedDirection, reflectedDirection };

						color += traceRay(reflectedRay, recurseLevel - 1) * hit.obj->reflection;
					}
