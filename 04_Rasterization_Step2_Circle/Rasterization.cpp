...
Rasterization::Rasterization(const int &width, const int &height)
    ...
    const size_t numTriangles = 432; // 삼각형 몇 개로 그릴지
    ...
    for (int i = 0; i < numTriangles; i++) {
        this->vertices.push_back(center + radius * vec3(glm::cos(theta), glm::sin(theta), 0.0f));
        this->colors.push_back(vec3((float(std::rand() % 100) / 100.0f), (float(std::rand() % 100) / 100.0f), (float(std::rand() % 100) / 100.0f)));
    ...
    //this->colors[colors.size() - 1] = this->colors[1];
}
