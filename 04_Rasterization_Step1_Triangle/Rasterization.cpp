...
Rasterization::Rasterization(const int &width, const int &height)
    ...
    // 시계방향인지 반시계방향인지 주의
    // cout << glm::cos(2.0f * 3.141592f / 6);//////////////////

    const int trianglesCount = 432;

    const auto radius = 1.0f;                   // 원의 반지름
    ...
    for (int i = 0; i < trianglesCount; i++) {
        ...
        triangle.v0.pos = center;
        triangle.v2.pos = center + radius * vec3(std::cos(theta), std::sin(theta), 0.0f);   // 시계방향의 꼭짓점 주의!

        theta += 2.0f * 3.141592f / float(trianglesCount);

        triangle.v1.pos = center + radius * vec3(std::cos(theta), std::sin(theta), 0.0f);
...
void Rasterization::Render(vector<vec4> &pixels) {
    ...
    for (int a = 0; a < triangles.size(); a++) { ///////////////////////////////////////////
        ...
        const auto v0 = ProjectWorldToRaster(triangles[a].v0.pos);
        const auto v1 = ProjectWorldToRaster(triangles[a].v1.pos);
        const auto v2 = ProjectWorldToRaster(triangles[a].v2.pos);
        ...
        const auto xMin = glm::clamp(glm::floor(std::min({v0.x, v1.x, v2.x})),
                                     0.0f, float(width - 1));
        const auto yMin = glm::clamp(glm::floor(std::min({v0.y, v1.y, v2.y})),
                                     0.0f, float(height - 1));
        const auto xMax = glm::clamp(glm::ceil(std::max({v0.x, v1.x, v2.x})),
                                     0.0f, float(width - 1));
        const auto yMax = glm::clamp(glm::ceil(std::max({v0.y, v1.y, v2.y})),
                                     0.0f, float(height - 1));
        ...
        for (size_t j = yMin; j <= yMax; j++) {
            for (size_t i = xMin; i <= xMax; i++) {
                ...
                if (alpha0 >= 0.0f && alpha1 >= 0.0f && alpha2 >= 0.0f) {
                    ...
                    const vec3 color = alpha0 / sum * triangles[a].v0.color +
                                       alpha1 / sum * triangles[a].v1.color +
                                       alpha2 / sum * triangles[a].v2.color;
