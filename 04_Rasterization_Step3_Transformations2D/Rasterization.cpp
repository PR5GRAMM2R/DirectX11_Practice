...
vec3 RotateAboutZ(const vec3 &v, const float &theta) { 
   /* vec3 rotateWeight = vec3(0.0f);

    rotateWeight.x = v.x * glm::cos(theta) - v.y * glm::sin(theta);
    rotateWeight.y = v.x * glm::sin(theta) + v.y * glm::cos(theta);*/

    return vec3(v.x * glm::cos(theta) - v.y * glm::sin(theta), v.x * glm::sin(theta) + v.y * glm::cos(theta), v.z);
}
...
void Rasterization::Update() {
    ...
    /*for (size_t i = 0; i < circle.vertices.size(); i++) {
        this->vertexBuffer[i] = circle.vertices[i] + this->translation1;
    }*/ 
    ...
    // 스케일(Scale)
    /*for (size_t i = 0; i < circle.vertices.size(); i++) {
        this->vertexBuffer[i].x = circle.vertices[i].x * this->scaleX;
        this->vertexBuffer[i].y = circle.vertices[i].y * this->scaleY;
    }*/

    for (size_t i = 0; i < circle.vertices.size(); i++) {
        auto temp = circle.vertices[i];

        temp = temp + this->translation1;
        temp = RotateAboutZ(temp, this->rotation1);
        temp = vec3(temp.x * this->scaleX, temp.y * this->scaleY, temp.z);

        temp = temp + this->translation2;

        /*auto temp2 = circle.vertices[i] + this->translation1;
        temp2 = RotateAboutZ(temp2, this->rotation1);*/


        temp = circle.vertices[0] + RotateAboutZ(temp, this->rotation2);
